...
\section{Evolution of Artificial Intelligence in Sports Analytics}
\gls{cv} has fundamentally altered the landscape of sports analytics, transitioning
...
``hand-crafted'' features saw the introduction of algorithms such as the \gls{hog}, \gls{sift}, and various optical flow techniques.
...
milestone demonstrated that hierarchical feature extraction through Deep \gls{cnn}s could outperform any human-engineered descriptor. Building on the foundations
...
As noted by~\cite{Zhou2025}, \gls{ai}-driven systems are now the standard for extracting kinematic
data and estimating pose in real time, but their application in swimming remains a frontier of
\gls{cv}. The current paradigm is moving toward Foundation Models, defined as large-scale
models trained on massive, diverse datasets that exhibit zero-shot
generalization~\cite{dosovitskiy2020image}. This thesis utilizes models such as \gls{sam}~2 and
...
to compensate for this distortion, often necessitating specialized refractive camera models or
...
swimming involves a unique multiphase appearance problem: an athlete alternates between
...
\gls{coco}~\cite{lin2014microsoft} or the MPII Human Pose dataset~\cite{andriluka20142d}, are largely
inadequate for the aquatic domain. While a model trained on \gls{coco} may excel at identifying a
pedestrian on a pavement, it lacks exposure to the blurred, refracted, and partially occluded
silhouettes of a submerged athlete. Einfalt et al.~\cite{einfalt2018activity} demonstrate that
even fine-tuned \gls{cnn}s evaluated on real swimming footage exhibit systematic error modes in the
aquatic environment, confirming that models pre-trained on terrestrial data generalize poorly to
this domain. This necessitates the use of temporal memory mechanisms, such as the memory bank
implemented in \gls{sam}~2~\cite{ravi2024sam}, which allows a system to maintain object features even
...
The subsequent shift toward \gls{cnn}s, including U-Net and earlier \gls{yolo} variants, offered better robustness to noise. In the context of pose estimation, this era
...
identification, providing a stable crop for subsequent analysis. Nevertheless, \gls{cnn}s are often
...
These limitations led to the adoption of \gls{vit}. Unlike convolutions, the
self-attention mechanism in \gls{vit}~\cite{dosovitskiy2020image} allows every token in the frame to
...
``plain \gls{vit}'' backbone and simple decoder head, an architectural choice that prioritizes feature
...
extraction, notably the \gls{sam}~\cite{kirillov2023segment} and
DINO~\cite{caron2021emerging}. \gls{sam}~2 extends segmentation to the temporal domain through a
...
water-air interface represents a meaningful advance over the \gls{cnn} era.

\section{The Challenge of Metric Scaling and Calibration}
A fundamental limitation in many \gls{cv}-based swimming systems is the conversion of distance from
...
3D multi-camera calibration, often using \gls{dlt}. As the constraints
...
difficulty of camera synchronization, making this approach impractical in routine coaching
environments.

Alternative paradigms include wearable \gls{imu}s, which can provide
direct acceleration data. However, \gls{imu}s face significant challenges in swimming, including data
contamination from water resistance and the fact that they are often prohibited in official
competitions. This leaves \gls{cv}-based planar homography as the most viable alternative. Homography
assumes a mapping between two planes, for example the plane of the swimmer's lane and the camera
sensor. While~\cite{Tran2024} utilized lane ropes for this purpose, discrete, underwater markers
...
the ``planar world'' assumption, such as the swimmer moving deeper into the pool than the
calibration plane, can lead to significant inaccuracies in stroke length and speed calculations.
This project extends the logic of marker-based calibration by using \gls{sam}~2 to track these markers
...
datasets such as \gls{coco}, which contains over 200{,}000 labelled
...
lack of standardized annotation protocols for the aquatic domain.

A solution emerging in recent literature is \gls{mitl} annotation, which sits
within the broader landscape of semi-supervised learning. Traditional semi-supervised techniques
such as pseudo-labelling~\cite{lee2013pseudo} and consistency regularization (for example,
FixMatch~\cite{sohn2020fixmatch}) attempt to leverage unlabelled data by having a model predict
its own labels. \gls{mitl} refines this by using high-capacity Foundation Models to generate
pseudo-labels. For example, by using \gls{sam}~2's zero-shot segmentation to identify markers in a
...
However, \gls{mitl} carries risks, the primary concern being ``confirmation bias'', whereby the
student model (for example, \gls{yolo}) learns to replicate the systematic errors of the teacher model
(for example, \gls{sam}~2). This project mitigates such risks through human-in-the-loop verification,
ensuring that the automated \gls{mitl} pipeline produces markers that are anatomically and
...
This research bridges these gaps by synthesizing a multi-stage pipeline that leverages recent
advances in Foundation Models. By using \gls{sam}~2 for stable temporal segmentation, the challenge of
...
properties of the medium. Finally, by employing an automated \gls{mitl} pipeline, the work demonstrates
a scalable method for generating domain-specific data without the prohibitive cost of solely manual
annotation. Ultimately, this work moves beyond qualitative video analysis toward an approximately metric-accurate
