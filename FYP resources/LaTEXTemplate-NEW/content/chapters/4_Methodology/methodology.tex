...
sufficient for a YOLO-family one-stage detector to localize accurately. A smaller segment
...
% ============================================================
\section{Dataset Generation and Model-in-the-Loop Annotation}\label{sec:dataset-generation}
% ============================================================

To address the second objective, necessitated by the lack of annotated underwater datasets, a
``Model-in-the-Loop'' (MITL) workflow was developed. This phase utilized foundation models to
...
domain adaptation. The MITL approach, by contrast, exploited the zero-shot generalization
...
The Segment Anything Model 2 (SAM 2)~\cite{ravi2024sam} was utilized to propagate marker
...
The system utilizes the Interquartile Range (IQR) method~\cite{tukey1977exploratory} to
...
verification tool was utilized to perform a final audit of the filtered dataset. This tool, shown in Figure~\ref{fig:bbox_editor}, implemented using the Tkinter framework, allowed for the rapid visual inspection of the SAM-generated labels.
...
one-stage YOLO variant provides sufficient localization accuracy at a fraction of the
...
process utilized a Distribution Focal Loss (DFL) gain of $1.5$ and a Box Loss gain of $7.5$.
...
on the SAM-generated dataset, several regularization and augmentation strategies were
employed.
...
rate scheduler had converged. The model utilized Mosaic augmentation ($1.0$) and Random
...
refractive noise. To stabilize the scale, the system implements a Median Absolute Deviation
...
The final phase synthesizes the outputs of marker detection, ego-motion compensation, and pose
...
refraction-aware MAD inlier filter was applied to stabilize the per-frame pixel-to-metre scale
