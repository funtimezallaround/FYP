...
\section{Marker Detection Accuracy}\label{sec:marker_detection}
...
\begin{figure}[htbp]
    \centering
        \centering
        \includegraphics[width=\linewidth]{image_011a7b.png}
        \includegraphics[width=\linewidth]{image_011a60.png}
        \includegraphics[width=\linewidth]{image_0116a2.png}
        \includegraphics[width=\linewidth]{image_01169b.png}
    \caption{Comparison of marker detection accuracy. Left: Inference on test frames from the training video, showing high confidence and precision. Right: Inference on an unseen video from the same pool.\label{fig:marker_detection_comparison}}
\end{figure}

Consequently, the model experiences variations in confidence scores and occasional bounding box jitter in the unseen footage. This highlights a common limitation in underwater \gls{cv}: high sensitivity to environmental fluctuations. These variations directly impact the stability of the frame-to-frame pixel-to-metre scale estimation. The subsequent application of the \gls{mad}-based inlier filter and Kalman smoothing is therefore critical to mitigate these detection inconsistencies and prevent them from propagating as severe positional drift.

\section{Trajectory Optimization and Pre-Evaluation Processing}\label{sec:trajectory_optimisation}

\subsection{Outlier Removal}
Prior to evaluation, the raw tracking records were subjected to the \gls{mad}-based inlier filter and temporal jump filter described in Chapter~\ref{chp:methodology}. Of the 4,052 raw records, 723 were flagged as significant deviations and removed, leaving 3,329 records for downstream processing. A secondary velocity ceiling of 5.0 m/s was subsequently applied, removing a further 32 records whose implied speed was physiologically implausible for competitive breaststroke swimming, justified by world-record 50 m breaststroke splits, which correspond to mean speeds of approximately 1.92 m/s~\cite{worldaquatics}. Any inferred speed exceeding 5.0 m/s therefore constitutes an unambiguous tracking artefact.
...
\begin{figure}[htbp]
    ...
    \caption{Full 50 m trajectory evaluation: system trajectory (black line) versus manually recorded ground truth split times (red points). The y-axis represents the distance from the origin (start wall). The swimmer travels from 0 m to 25 m (outbound) and returns to 0 m.\label{fig:trajectory_eval}}
\end{figure}
...
\begin{table}[htbp]
    \centering
    \caption{Raw vs.\ Kalman-filtered standard deviation for position and scale estimates.}\label{tab:kalman_noise}
    \begin{tabular}{lrr}
        \hline
...
Despite the inherent noise in individual trajectories, evaluating the coordination between limbs provides valuable insights. Because breaststroke dictates simultaneous and symmetrical arm movements, the relationship between the left and right wrists was visualized using a heatmap (Figure~\ref{fig:wrist_symmetry}), where both wrists should exhibit similar motion patterns. In an ideal stroke with perfect tracking, the distribution would manifest as a highly concentrated, overlapping cluster centred around the same relative position for both wrists. The observed heatmap, however, shows a more dispersed pattern with multiple density peaks, resulting from the noise from before the swim start, and from the shift in the swimmer's position for the turn, push-off and underwater glide phases. 

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\textwidth]{wrist_symmetry_heatmap.png}
    \caption{Symmetry heatmap illustrating the spatial correlation between the left and right wrist motions relative to the swimmer's camera-facing shoulder. High-density areas indicate the most common spatial relationship between the limbs during the stroke cycle.\label{fig:wrist_symmetry}}
\end{figure}
